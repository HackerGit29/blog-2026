...
import { Globe, Instagram, Linkedin, Github, Twitter, Music2, Calendar, Users } from 'lucide-react';
...
export function AProposTab({ username }: { username?: string }) {
  ...
  const links = Object.entries(LINK_LABELS)
    .map(([key, label]) => {
      ...
      if (!url) return null;
      return { key, label, url, icon: LINK_ICONS[key] };
    })
          ...
          <Box sx={{ mb: 5 }}>
            {links.map((link) => (
              <Box key={link.key} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, py: 0.8 }}>
                <Box sx={{ color: 'text.secondary', display: 'flex', flexShrink: 0 }}>{link.icon}</Box>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
        {username && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            <Box sx={{ color: 'text.secondary', display: 'flex' }}><Globe size={16} /></Box>
            <Typography variant="body2" sx={{ color: 'text.secondary', minWidth: 80, fontWeight: 500 }}>Chaine</Typography>
            <Link href={'/' + username} sx={{ color: '#4FC3F7', textDecoration: 'none', fontSize: '0.875rem', '&:hover': { textDecoration: 'underline' } }}>
              /{username}
            </Link>
          </Box>
        )}
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Box sx={{ color: 'text.secondary', display: 'flex' }}><Users size={16} /></Box>
          <Typography variant="body2" sx={{ color: 'text.secondary', minWidth: 80, fontWeight: 500 }}>Abonnes</Typography>
          <Typography variant="body2" sx={{ color: 'text.primary' }}>{profile.formattedFollowers || '0'}</Typography>
        </Box>
      </Box>
    </Box>
  );
