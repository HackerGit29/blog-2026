...
--
-- Goal:
--   - Distinct 'superadmin' role (separate from 'admin') for the tenant owner
--   - 'superadmin' bypasses all RLS (moderation, ban, badge-grant capability)
--   - 'admin' scoped to their own author_id (unchanged)
--   - 'is_banned' flag blocks all reads/writes for the user (auth, profile, content)

-- 1. Extend the app_role enum with 'superadmin'
alter type app_role add value if not exists 'superadmin';

-- 2. is_banned on user_profiles
alter table user_profiles add column if not exists is_banned boolean not null default false;

-- 3. Updated grant_superadmin: assigns 'superadmin' role to the canonical owner
...
-- 4. Helper: current user is superadmin (used inside RLS policies)
...
-- 5. Helper: current user is banned
...
-- 6. RLS: superadmin can manage all content, banned blocked from all
-- admin_articles
...
-- 7. user_profiles: users read own, superadmin reads/writes all, banned blocked
...
-- 8. user_roles: only superadmin can manage
...
-- 9. Apply: ensure admin@akadev.site is superadmin
