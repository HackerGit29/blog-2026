...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const url = `${env.SUPABASE_URL}/rest/v1/admin_articles?select=slug,created_at&status=eq.published&order=published_at.desc`;
    ...
    const articles = await res.json() as { slug: string; created_at: string | null }[];
